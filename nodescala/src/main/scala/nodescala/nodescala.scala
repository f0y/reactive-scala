...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    try {
    response.foreach(r =>
      if (token.nonCancelled) exchange.write(r) else throw new CancellationException
    )} finally {
      exchange.close()
    }
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val mainLoopSubs = Future.run() {
      ct => Future {
        while (ct.nonCancelled) {
          val request = listener.nextRequest()
          request onComplete {
            case Success((req, exchange)) =>
              Future {respond(exchange, ct, handler(req))}
            case Failure(f) => throw f
          }
        }
      }
    }
    Subscription(listenerSubs, mainLoopSubs)
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      val f = p.future
      createContext(ex => {
        p.tryComplete(Try(ex.request, ex))
      })
