...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs.foldRight(Future.always(List.empty[T]))( {
        (f, list) =>  f.flatMap(
          value => list.map(
            rawList => {value :: rawList}))
      })
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.foreach(f => {
        f.onComplete {
          case Success(y) => p.tryComplete(Try(y))
          case Failure(z) =>  p.failure(z)
        }
      })
      p.future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      val token = cancellationTokenSource.cancellationToken
      f(token)
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete({
        case Success(v) => p.tryComplete(Try(cont(f)))
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = promise[S]()
      f onComplete  {
        case Success(v) => p.complete(Try(cont(Try(v))))
        case Failure(fail) => p failure fail
      }
      p.future
    }
